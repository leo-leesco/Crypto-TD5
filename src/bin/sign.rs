...
fn sign(sk: [u8; 32], message: &[u8]) -> (U256, U256) {
    let (x, secret) = secret_expand(sk);
...
fn main() {
    ...
    let sk: [u8; 32] = read(
        args.nth(1)
            .expect("Please provide the path to the secret key"),
    )
    .expect("Could not read the secret key file")
    .try_into()
    .expect("Could not parse the secret key as a 32-byte key");
